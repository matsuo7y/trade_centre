...
from ..indicator import IndicatorType, MACDIndicator, MACDIndicatorSign, BBANDIndicator, BBANDIndicatorSign
from ..progress_recorder import MACDRecorder, BBANDRecorder


class BBandMACDTradeStrategy(AbstractTradeStrategy):
    ...
    def indicator_builder_adder(self):
        self.indicator_builder.add(
            IndicatorType.BBAND.name,
            BBANDIndicator(time_period=12, is_test=self.is_test)
        )
        self.indicator_builder.add(
    ...
    def progress_recorders_appender(self):
        self.progress_recorders.append(BBANDRecorder())
